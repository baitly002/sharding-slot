package com.rlynic.sharding.slot.database.context;
...
import org.apache.shardingsphere.sql.parser.sql.common.util.*;
...
@Getter
@Setter
public final class SelectInStatementContext extends CommonSQLStatementContext<SelectStatement> implements TableAvailable, WhereAvailable, ParameterAware {
    ...
    public SelectInStatementContext(final Map<String, ShardingSphereDatabase> databases, final List<Object> parameters, final SelectStatement sqlStatement, final String defaultDatabaseName) {
        super(sqlStatement);
        ...
        selectInContext = new SelectInContextEngine().createSelectInContext(sqlStatement);
    ...
    private Map<Integer, org.apache.shardingsphere.infra.binder.statement.dml.SelectStatementContext> createSubqueryContexts(final Map<String, ShardingSphereDatabase> databases, final List<Object> parameters, final String defaultDatabaseName) {
        Collection<SubquerySegment> subquerySegments = SubqueryExtractUtil.getSubquerySegments(getSqlStatement());
        Map<Integer, org.apache.shardingsphere.infra.binder.statement.dml.SelectStatementContext> result = new HashMap<>(subquerySegments.size(), 1);
        for (SubquerySegment each : subquerySegments) {
            org.apache.shardingsphere.infra.binder.statement.dml.SelectStatementContext subqueryContext = new org.apache.shardingsphere.infra.binder.statement.dml.SelectStatementContext(databases, parameters, each.getSelect(), defaultDatabaseName);
